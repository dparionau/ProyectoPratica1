...
# ARCHIVO LOCAL DONDE SE GUARDARÁN LOS DATOS PERMANENTEMENTE EN EL DISCO
...
st.markdown("""
    ...
    [data-testid="stForm"], div[data-testid="stExpander"] {
        ...
        box-shadow: 0 4px 20px rgba(0, 0, 0, 0.4) !important; margin-bottom: 15px;
    }
    div[data-testid="stExpander"] summary {
        color: #38bdf8 !important; font-weight: 600; font-size: 1.05rem;
    }
...
# =========================================================
    ...
    with st.expander("📋 Ver Registro e Histórico de Transacciones", expanded=True):
...
# =========================================================
    ...
    with st.expander("📊 Inventario Consolidado y Valorización", expanded=True):
...
# =========================================================
    ...
    with st.expander("📜 Bitácora de Cálculos Registrados", expanded=False):
...
# =========================================================
    ...
    # DIRECTORIO
    with st.expander("👥 Directorio de Personal (LEER)", expanded=True):
    ...
    # ACTUALIZAR
    with st.expander("✏️ Editar Registro de Empleado (ACTUALIZAR)", expanded=False):
        if st.session_state.empleados_crud:
    ...
    # ELIMINAR
    with st.expander("🗑️ Eliminar Registro de Empleado (BORRAR)", expanded=False):
        if st.session_state.empleados_crud:
            ...
            if emp_del:
                st.warning(f"¿Confirma la eliminación permanente de **{emp_del['nombre']}** (ID: {id_del})?")
